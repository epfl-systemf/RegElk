...
type regex =
  ...
  | Re_quant of bool * capture * capture * lookid * lookid * quantifier * regex
...
let rec print_regex (r:regex) : string =
  ...
  | Re_quant (_, _, _, _, _, q, r1) -> print_regex r1 ^ print_quant q
...
let rec annotate_regex (ra:raw_regex) (c:capture) (l:lookid) : regex * capture * lookid =
  ...
  | Raw_quant (quant, r1) ->
     let (ar1, c1, l1) = annotate_regex r1 c l in
     (Re_quant (nullable ar1, c, c1, l, l1, quant, ar1), c1, l1)
  | Raw_capture r1 ->
...
let lazy_prefix (r:regex) : regex =
  Re_con (Re_quant (nullable r, 0, 0, 0, 0, LazyStar, Re_dot),r)
...
let rec reverse_regex (r:regex) : regex =
  ...
  | Re_quant (nul, cstart, cend, lstart, lend, quant, r1) -> Re_quant (nul, cstart, cend, lstart, lend, quant, reverse_regex r1)
...
let rec remove_capture (r:regex) : regex =
  ...
  | Re_quant (nul, cstart, cend, lstart, lend, quant, r1) ->
     Re_quant (nul, cstart, cstart, lstart, lstart, quant, remove_capture r1)
...
let rec get_lookaround (r:regex) (lid:lookid) : (regex * lookaround) option =
  ...
  | Re_quant (_, _, _, _, _, _, r1) | Re_capture (_, r1)->
...
let rec max_lookaround (r:regex) : lookid =
  ...
  | Re_quant (_,_,_,_,_,_,r1) | Re_capture (_,r1) -> max_lookaround r1
...
let rec max_group (r:regex) : capture =
  ...
  | Re_quant (_,_,_,_,_,_,r1) | Re_lookaround (_,_,r1) -> max_group r1
