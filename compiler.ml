...
let full_compilation (r:regex) : compiled_regex =
  ...
  let looktypes = Array.make maxlook Lookahead in
  let lookcdns = Array.make maxlook [] in
  let lookast = Array.make maxlook Re_empty in
  let build_look = Array.make maxlook empty_code in
