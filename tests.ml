...
(* Of course here we restrict this list to the paper examples that can be executed by this version (no lookaheads for instance). For the full version with all examples, see the main development directory *)
let paper_tests : (raw_regex*string) list =
  [(Raw_con(Raw_quant(Star,Raw_char('a')),Raw_char('b')),"caabd"); (* (a* )b *)
   ...
   (Raw_con(Raw_capture(Raw_alt(Raw_char('a'),Raw_dot)),Raw_char('b')),"ab"); (* (a|.)b *)
   (Raw_con(Raw_quant(Star,Raw_capture(Raw_quant(Plus,Raw_char('a')))),Raw_char('b')),"aaa"); (* (a+)*b *)
   (Raw_quant(Star,Raw_capture(Raw_alt(Raw_capture(Raw_char('a')),Raw_capture(Raw_char('b'))))),"ab");                     (* ((a)|(b))* *)
   (Raw_quant(Star,Raw_con(Raw_alt(Raw_char('a'),Raw_empty),Raw_alt(Raw_empty,Raw_char('b')))),"ab"); (* (?:(?:a|)(?:|b))* *)
   (Raw_quant(Star,Raw_alt(Raw_quant(Star,Raw_alt(Raw_capture(Raw_char('a')),Raw_char('b'))),Raw_char('c'))),"ac"); (* (?:(?:(a)|b)*|c)* *)
   (Raw_quant(Star,Raw_alt(Raw_quant(Star,Raw_capture(Raw_char('a'))),Raw_quant(Star,Raw_alt(Raw_capture(Raw_char('b')),Raw_capture(Raw_char('c')))))),"abc"); (* (?:(a)*|(?:(b)|(c))* )* *)
   (Raw_con(Raw_con(Raw_con(Raw_char('a'),Raw_char('b')),Raw_char('c')),Raw_lookaround(Lookbehind,Raw_con(Raw_con(Raw_con(Raw_char('a'),Raw_char('b')),Raw_lookaround(Lookbehind,Raw_char('b'))),Raw_char('c')))),"abc"); (* abc(?<=ab(?<=b)c) *)
   (Raw_quant(Plus,Raw_alt(Raw_empty,Raw_dot)),""); (* (?:|.)+ *)
  ]
